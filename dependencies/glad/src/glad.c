/*
	...
	OpenGL, OpenGL ES loader generated by glad 0.1.36 on Sat Aug  2 15:20:13 2025.
...
static int get_exts(void)
{
	...
	else
	{
		...
		{
			const char *gl_str_tmp = (const char *)glGetStringi(GL_EXTENSIONS, (unsigned int)index);
...
{
	if (!GLAD_GL_ARM_shader_core_properties)
		return;
...
{
	if (!GLAD_GL_EXT_texture_storage_compression)
		return;
...
{
	if (!GLAD_GL_MESA_sampler_objects)
		return;
	glad_glGenSamplers = (PFNGLGENSAMPLERSPROC)load("glGenSamplers");
	glad_glDeleteSamplers = (PFNGLDELETESAMPLERSPROC)load("glDeleteSamplers");
	glad_glIsSampler = (PFNGLISSAMPLERPROC)load("glIsSampler");
...
{
	if (!GLAD_GL_OES_EGL_image_external)
		return;
	glad_glEGLImageTargetTexture2DOES = (PFNGLEGLIMAGETARGETTEXTURE2DOESPROC)load("glEGLImageTargetTexture2DOES");
}
...
{
	if (!GLAD_GL_QCOM_frame_extrapolation)
		return;
...
{
	if (!GLAD_GL_QCOM_motion_estimation)
		return;
