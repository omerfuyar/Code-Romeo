...
#define eAssertEntity(entity) RJ_DebugAssert((entity) < ENTITY.info.count + ENTITY.info.freeIndices.count && entity != RJ_INDEX_INVALID && eIsActive(entity), "Entity %u either exceeds maximum possible index %u, invalid or inactive.", (entity), ENTITY.info.count + ENTITY.info.freeIndices.count)

#pragma endregion SourceOnly

RJ_ResultWarn Entity_Initialize(RJ_Size initialEntityCapacity)
{
    ENTITY.info.capacity = initialEntityCapacity;
    ENTITY.info.count = 0;

    ListArray_Create(&ENTITY.info.freeIndices, "Entity Free Indices", sizeof(RJ_Size), ENTITY_INITIAL_FREE_INDEX_ARRAY_SIZE);

    RJ_ReturnAllocate(Vector3, ENTITY.data.positions, initialEntityCapacity,
                      ListArray_Destroy(&ENTITY.info.freeIndices);
                      ENTITY.info.capacity = 0;);
...
{
    RJ_DebugAssert(ENTITY.info.count + ENTITY.info.freeIndices.count < ENTITY.info.capacity, "Maximum Entity capacity of %u reached.", ENTITY.info.capacity);

    Entity newEntity = ENTITY.info.freeIndices.count != 0 ? *((RJ_Size *)ListArray_Pop(&ENTITY.info.freeIndices)) : ENTITY.info.count;
...
void Entity_Destroy(Entity entity)
{
    ...
    ListArray_Add(&ENTITY.info.freeIndices, &entity);
